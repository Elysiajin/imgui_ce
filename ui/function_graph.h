...
//   3) 向后回溯：向前最多 0x400 字节收集函数边界（ret C3 / int3 / nop 填充
//      之后的位置），从最早的边界做线性解码验证（跳转只计长度、不跨越
//      ret/int3），恰好落在目标地址的最早边界即函数头；
//   4) 边界回溯失败 → ≤0x1000 内最近导出符号，且线性能解码到达目标；
//   5) 兜底：地址本身（CE 在无任何信息时同样从选中地址开始解析）。
// 函数体识别：从入口递归下降（工作队列），标准基本块语义 —— ret / 无条件
// jmp / 条件跳转(jcc) 都终止当前块；jcc 的 taken 与 fall-through 两个方向
// 各自成块并连边（跳转指令不会出现在块中部）。跳到已解析块则连边收敛；
